...
public class GDFDirTable : List<GDFDirEntry>, ICloneable
{
    ...
    public GDFDirTable(CBinaryReader File, GDFVolumeDescriptor Vol, uint Sector, uint Size)
    {
        this.Sector = Sector;
        this.Size = Size;
        File.Seek((Sector * (long)Vol.SectorSize) + Vol.RootOffset, SeekOrigin.Begin);
        byte[] array = File.ReadBytes((int)Size);
        MemoryStream memoryStream = new(array);
        CBinaryReader cBinaryReader = new(EndianType.LittleEndian, memoryStream);
        try
        {
    ...
    {
        byte[] array = new byte[(int)(Math.Ceiling(Size / 2048.0) * 2048.0)];
        MemoryStream memoryStream = new(array);
    ...
    private static void updateInOrder(GDFEntryNode Node, ref uint offset)
    {
    ...
    private static void writeInOrder(GDFEntryNode Node, MemoryStream ms)
    {
    ...
    public void CreateBST()
    {
        ...
        Sort();
        GDFDirEntry gDFDirEntry = base[(int)Math.Floor(Count / 2.0)];
