...
public partial class FileRenamer
{
    ...
    static readonly string[] VersionPlaceholders = ["{Version}"];
    static readonly string[] DLCPlaceholders = ["{DLCCount}"];
    ...
    private static string RenameFile(string sourcePath, string newFileName)
    {
        try
        {
            ...
            string newFileNameCleaned = new(newFileName
                ...
                .ToArray());
            newFileNameCleaned = RemoveDoubleSpaceRegex().Replace(newFileNameCleaned, " ");
    ...
    [GeneratedRegex(@"\s+")]
    private static partial Regex RemoveDoubleSpaceRegex();
}
